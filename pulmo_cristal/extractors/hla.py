...
class HLAExtractor(BaseExtractor):
    ...
    def _extract_hla_with_regex(self, text: str) -> Dict[str, str]:
        ...
        # Pattern pour DQA et DQB avec support pour deux valeurs
        try:
            # DQA pattern - chercher deux valeurs
            dqa_pattern = r"DQA\s+DQA\s*\n\s*(\d+)\s+(\d+)"
            dqa_matches = re.findall(dqa_pattern, text, re.DOTALL)
            if dqa_matches and len(dqa_matches[0]) >= 2:
                hla_data.update({
                    "DQA1": self._clean_hla_value(dqa_matches[0][0]),
                    "DQA2": self._clean_hla_value(dqa_matches[0][1]),
                })
            
            # DQB pattern - chercher deux valeurs  
            dqb_pattern = r"DQB\s+DQB\s*\n\s*(\d+)\s+(\d+)"
            dqb_matches = re.findall(dqb_pattern, text, re.DOTALL)
            if dqb_matches and len(dqb_matches[0]) >= 2:
                hla_data.update({
                    "DQB1": self._clean_hla_value(dqb_matches[0][0]),
                    "DQB2": self._clean_hla_value(dqb_matches[0][1]),
                })
                
            # Si pas trouvé avec le pattern standard, essayer dans le tableau principal
            if not dqb_matches:
                # Chercher dans le tableau principal
                main_table_pattern = r"A1\s+A2\s+B1\s+B2\s+.*?DR1\s+DR2\s+DQB[^\n]*\n\s*\d+\s+\d+\s+\d+\s+\d+\s+.*?\d+\s+\d+\s+(\d+)"
                main_matches = re.findall(main_table_pattern, text, re.DOTALL)
                if main_matches:
                    hla_data["DQB1"] = self._clean_hla_value(main_matches[0])
                    
                # Chercher la deuxième valeur DQB
                hla_dqb_pattern = r"HLA DQB\s*\n\s*(\d+)"
                hla_dqb_matches = re.findall(hla_dqb_pattern, text, re.DOTALL)
                if hla_dqb_matches:
                    hla_data["DQB2"] = self._clean_hla_value(hla_dqb_matches[0])

        except re.error as e:
            self.log(f"Error in DQA/DQB regex: {str(e)}", level=logging.ERROR)
    ...
    def _clean_hla_header(self, header: str) -> str:
        ...
        # Tryf to match with known patterns
    ...
    def _create_standardized_hla(self, hla_data: Dict[str, str]) -> Dict[str, str]:
        """
        Create a standardized HLA dictionary with split DQA/DQB fields.
        """
        standard_keys = [
            "A1", "A2", "B1", "B2", "C1", "C2", 
            "DR1", "DR2", "DQA1", "DQA2", "DQB1", "DQB2", 
            "DP1", "DP2"
        ]
