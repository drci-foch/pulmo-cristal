...
class DonorPDFExtractor(PDFExtractor):
    ...
    def extract_donor_data(self, pdf_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        for section_name in [
            ...
            "bilan_pulmonaire", "parametres_respiratoires", "bilan_cardiaque_morphologique",
            "thorax"
        ]:
            ...
            try:
                ...
                self.log(f"Pattern group '{pattern_group}' not found. Skipping section.", 
                         level=logging.WARNING)
                result[section_name] = {}
            except Exception as e:
                self.log(f"Error extracting section '{section_name}': {str(e)}", 
                         level=logging.ERROR)
                result[section_name] = {}
        ...
        try:
            ...
            for key, pattern in donor_patterns.items():
                try:
                    ...
                    self.log(f"Error extracting donor field '{key}': {str(e)}", 
                             level=logging.WARNING)
            
            # Try alternative patterns for missing fields
            if "type_donneur" not in result or not result["type_donneur"] or result["type_donneur"] == "::":
                try:
                    ...
                    self.log(f"Error extracting alternative donor type: {str(e)}", 
                             level=logging.WARNING)
        
        except KeyError:
            self.log("Donor patterns not found. Basic info extraction failed.", 
                     level=logging.ERROR)
        except Exception as e:
            self.log(f"Unexpected error in basic info extraction: {str(e)}", 
                     level=logging.ERROR)
            ...
            try:
                ...
                if matches:
                    ...
                    if re.search(r"([0-9.,]+)\s*(mmol|UI|µmol|mg)", pattern.pattern):
                        ...
                        result[key] = numeric_values if len(numeric_values) > 1 else numeric_values[0]
                    else:
                        # For text values
                        if isinstance(first_match, tuple) and len(first_match) > 0:
                            result[key] = first_match[0].strip()
                ...
                self.log(f"Error extracting pattern '{key}': {str(e)}", 
                         level=logging.WARNING)
