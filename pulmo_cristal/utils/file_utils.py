...
def list_directory_tree(directory: Union[str, Path], 
                      max_depth: int = 3, 
                      indent: str = "  ",
    ...
    def _list_dir(path: Path, prefix: str, depth: int) -> Tuple[int, int]:
        ...
        for i, item in enumerate(items):
            ...
            if item.is_dir():
                ...
                if file_types is None or any(item.name.lower().endswith(ext.lower()) for ext in file_types):
                    result.append(f"{prefix}{branch}{item.name}")
                    file_count += 1
...
def batch_process_files(files: List[Union[str, Path]], 
                       batch_size: int = 10, 
                       logger: Optional[logging.Logger] = None) -> Generator[List[Path], None, None]:
    """
...
def create_versioned_filename(base_name: str, 
                            extension: str, 
                            version: str = "0.1.0",
                            add_timestamp: bool = True,
