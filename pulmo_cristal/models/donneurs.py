...
class HLAData:
    ...
    def is_valid(self) -> bool:
        ...
        return all(getattr(self, field) not in ("", "À AJOUTER", "INCONNU") 
...
class BilanHemodynamiqueData:
    ...
    def from_dict(cls, data: Dict[str, str]) -> 'BilanHemodynamiqueData':
        ...
        for field_name, field_value in data.items():
            if hasattr(result, field_name):
                if field_name in ["concentre_globulaire", "plasma_frais_congele", "concentre_plaquettaire"]:
                    try:
...
class BilanPulmonaireData:
    ...
    def from_dict(cls, data: Dict[str, str]) -> 'BilanPulmonaireData':
        ...
        for field_name, field_value in data.items():
            ...
            if hasattr(result, field_name):
                if field_name in ["radiographie_thoraco_pulmonaire", "aspirations_tracheo_bronchiques"]:
                    setattr(result, field_name, str(field_value).strip())
                else:
                    setattr(result, field_name, BooleanValue.from_string(field_value))
...
class Donneur:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Donneur':
        ...
        if "date_extraction" in data:
            try:
                donneur.date_extraction = datetime.fromisoformat(data["date_extraction"])
            except (ValueError, TypeError):
                try:
        ...
        if info_donneur:
            ...
            donneur.type_donneur = DonneurType.from_string(info_donneur.get("type_donneur", ""))
        ...
        if "bilan_infectieux" in data:
            donneur.bilan_infectieux = BilanInfectieuxData.from_dict(data["bilan_infectieux"])
            
        if "bilan_hemodynamique" in data:
            donneur.bilan_hemodynamique = BilanHemodynamiqueData.from_dict(data["bilan_hemodynamique"])
            
        if "evolution_hemodynamique" in data:
            donneur.evolution_hemodynamique = EvolutionHemodynamiqueData.from_dict(data["evolution_hemodynamique"])
            
        if "bilan_pulmonaire" in data:
            donneur.bilan_pulmonaire = BilanPulmonaireData.from_dict(data["bilan_pulmonaire"])
            
        if "parametres_respiratoires" in data:
            donneur.parametres_respiratoires = ParametresRespiratoiresData.from_dict(data["parametres_respiratoires"])
            
        if "bilan_cardiaque_morphologique" in data:
            donneur.bilan_cardiaque = BilanCardiaqueData.from_dict(data["bilan_cardiaque_morphologique"])
        elif "bilan_cardiaque" in data:
            donneur.bilan_cardiaque = BilanCardiaqueData.from_dict(data["bilan_cardiaque"])
    ...
    def validate(self) -> bool:
        ...
        if self.date_naissance and self.date_clampage:
            if self.date_naissance > self.date_clampage.date():
                self.validation_errors.append("Date de naissance postérieure à la date de clampage")
        
        if self.date_creation and self.date_clampage:
            if self.date_creation > self.date_clampage:
                self.validation_errors.append("Date de création postérieure à la date de clampage")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "date_entree_bloc": self.date_entree_bloc.isoformat() if self.date_entree_bloc else None,
            "date_clampage": self.date_clampage.isoformat() if self.date_clampage else None,
        ...
        for field_name in [
            "serologies", "morphologie", "habitus", "antecedents",
            "bilan_infectieux", "bilan_hemodynamique", "evolution_hemodynamique",
            "bilan_pulmonaire", "parametres_respiratoires", "bilan_cardiaque", "thorax"
        ]:
