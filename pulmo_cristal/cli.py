...
def setup_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version", "-V", 
        action="version", 
        version=f"pulmo-cristal {__version__}"
    )
    ...
    extract_parser.add_argument(
        "--pattern", "-p", 
        default=None,
        help="Regex pattern to filter input files"
    )
    ...
    list_parser.add_argument(
        "--pattern", "-p", 
        default=None,
        help="Regex pattern to filter listed files"
    )
    ...
    list_parser.add_argument(
        ...
        type=int, 
        default=3,
        help="Maximum depth for directory listing"
    )
    
    # Convert command
    convert_parser = subparsers.add_parser(
        "convert", 
        help="Convert between output formats"
    )
...
def extract_command(args: argparse.Namespace, logger: logging.Logger) -> int:
    ...
    json_filename = create_versioned_filename("donneurs_data", "json", __version__, True)
    ...
    for batch in batch_process_files(pdf_files, batch_size=args.batch_size, logger=logger):
        batch_data = []
        ...
        for pdf_file in batch:
            try:
                ...
                if "date_extraction" not in donor_data:
                    donor_data["date_extraction"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    logger.info(f"Processing complete: {total_success} successes, {total_failures} failures")
    logger.info(f"Total time: {elapsed_time:.2f} seconds ({elapsed_time/len(pdf_files):.2f} seconds per file)")
    ...
    for i, donor_dict in enumerate(donor_data):
        try:
            ...
            if is_valid:
                ...
                invalid_count += 1
                logger.warning(f"Validation errors in {result['id']}: {donneur.validation_errors}")
