...
class DonorJSONExporter(BaseExtractor):
    ...
    def export_json_stream(self,
        ...
        try:
            with open(output_path, 'w', encoding=self.encoding) as jsonfile:
                ...
                jsonfile.write('{\n')
                jsonfile.write(f'    "metadata": {json.dumps(metadata, ensure_ascii=self.ensure_ascii, indent=self.indent)},\n')
                jsonfile.write('    "donors": [\n')
                
                # Process data in batches
                for i, donor in enumerate(data_list):
                    # Convert to JSON with proper indentation
                    donor_json = json.dumps(donor, ensure_ascii=self.ensure_ascii, indent=self.indent)
                    
                    # Add indentation to each line to match the file structure
                    indented_lines = [f"        {line}" for line in donor_json.splitlines()]
                    donor_text = "\n".join(indented_lines)
    ...
    def validate_json(self, data: Dict[str, Any], schema: Optional[Dict[str, Any]] = None) -> List[str]:
        """
...
def generate_json_filename(base_name: str = "donneurs_data", include_version: bool = True,
